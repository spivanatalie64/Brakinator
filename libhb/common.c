    ...
    {
        {
            ...
            "nvenc_av1_10bit",
            "AV1 10-bit (NVEnc)",
            HB_VCODEC_FFMPEG_NVENC_AV1_10BIT,
            HB_MUX_MASK_MP4 | HB_MUX_MASK_WEBM | HB_MUX_MASK_MKV,
        },
        NULL,
        0,
    ...
    {
        {
            ...
            "ffv1",
            "FFV1 (libavcodec)",
            HB_VCODEC_FFMPEG_FFV1,
            HB_MUX_MASK_MP4 | HB_MUX_MASK_MKV | HB_MUX_AV_AVI,
        },
        NULL,
        0,
        1,
...
#if HB_PROJECT_FEATURE_MF
    // TODO: Try to instantiate a throwaway encoder to see if a suitable
    // MediaFoundation encoder can be found? Alt, implement logic similar to
    // ffmpeg's encoder selection, to see if one would be found.
...
const char *hb_audio_name_get_default(hb_channel_layout_t *ch_layout,
                                      int mixdown) {
  int mix_channels = 2;
...
const char *hb_audio_name_generate(const char *name,
                                   hb_channel_layout_t *ch_layout, int mixdown,
                                   int keep_name,
  ...
  if (name != NULL && (!strcmp(name, "Mono") || !strcmp(name, "Stereo") ||
...
const char *hb_audio_name_generate_s(const char *name, const char *layout,
                                     int mixdown, int keep_name,
  ...
  generated_name =
      hb_audio_name_generate(name, &ch_layout, mixdown, keep_name, behaviour);
...
int hb_video_hdr_dynamic_metadata_is_supported(uint32_t codec,
                                               int hdr_dynamic_metadata,
                                               int profile) {
  if (hdr_dynamic_metadata == HB_HDR_DYNAMIC_METADATA_HDR10PLUS) {
    ...
    if (profile != 5 && profile != 7 && profile != 8 && profile != 10) {
      return 0;
    }
...
static const char *const hb_empty_list_names[] = {"auto", NULL};
...
static const char *const hb_empty_tune_names[] = {"none", NULL};
...
static const enum AVPixelFormat standard_pix_fmts[] = {AV_PIX_FMT_YUV420P,
                                                       AV_PIX_FMT_NONE};

static const enum AVPixelFormat standard_422_pix_fmts[] = {AV_PIX_FMT_YUV422P,
                                                           AV_PIX_FMT_NONE};

static const enum AVPixelFormat standard_444_pix_fmts[] = {AV_PIX_FMT_YUV444P,
                                                           AV_PIX_FMT_NONE};
...
static const enum AVPixelFormat standard_12bit_pix_fmts[] = {
    AV_PIX_FMT_YUV420P12, AV_PIX_FMT_YUV420P10, AV_PIX_FMT_YUV420P,
    AV_PIX_FMT_NONE};
...
int hb_mixdown_is_supported(int mixdown, uint32_t codec,
...
int hb_mixdown_has_remix_support(int mixdown, hb_channel_layout_t *ch_layout) {
  ...
  case HB_AMIXDOWN_5_2_LFE:
    return (av_channel_layout_subset(ch_layout, AV_CH_FRONT_LEFT_OF_CENTER) &&
            ...
            av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_STEREO) ==
                AV_CH_LAYOUT_STEREO);

  // 7.0 or better
  case HB_AMIXDOWN_7POINT1:
    return (av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_7POINT0) ==
  ...
  case HB_AMIXDOWN_6POINT1:
    return (av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_7POINT0) ==
                AV_CH_LAYOUT_7POINT0 ||
            av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_6POINT0) ==
                AV_CH_LAYOUT_6POINT0 ||
            av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_HEXAGONAL) ==
                AV_CH_LAYOUT_HEXAGONAL);

  // stereo + either of front center, side or back left/right, back center
  case HB_AMIXDOWN_5POINT1:
    return (av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_2_1) ==
                AV_CH_LAYOUT_2_1 ||
            av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_2_2) ==
                AV_CH_LAYOUT_2_2 ||
            av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_QUAD) ==
                AV_CH_LAYOUT_QUAD ||
            av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_SURROUND) ==
  ...
  case HB_AMIXDOWN_DOLBYPLII:
    return (
        av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_2_1) ==
            AV_CH_LAYOUT_2_1 ||
        av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_2_2) ==
            AV_CH_LAYOUT_2_2 ||
        av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_QUAD) ==
            AV_CH_LAYOUT_QUAD ||
        (av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_STEREO_DOWNMIX) ==
             AV_CH_LAYOUT_STEREO_DOWNMIX &&
         ch_layout->nb_channels == 2 &&  // decavcodecaBSInfo tells us the input
                                         // signals matrix encoding
         mixdown == HB_AMIXDOWN_DOLBY)); // allows signaling matrix encoding in
                                         // output w/encoders that support it
   ...
   *
   * Thus specifically only allow them for non-matrix Stereo input (layout ==
   * STEREO).
   */
  case HB_AMIXDOWN_LEFT:
  case HB_AMIXDOWN_RIGHT:
    return av_channel_layout_subset(ch_layout, AV_CH_LAYOUT_STEREO) ==
               AV_CH_LAYOUT_STEREO &&
...
int hb_mixdown_get_best(uint32_t codec, hb_channel_layout_t *ch_layout,
                        int mixdown) {
  // Passthru, only "None" mixdown is supported
  if (codec & HB_ACODEC_PASS_FLAG)
...
int hb_layout_get_name(const hb_channel_layout_t *ch_layout, char *name,
                       int size) {
...
int hb_layout_get_low_freq_channel_count(const hb_channel_layout_t *ch_layout) {
  uint64_t subset = av_channel_layout_subset(
      ch_layout, AV_CH_LOW_FREQUENCY | AV_CH_LOW_FREQUENCY_2);
  return !!(subset & AV_CH_LOW_FREQUENCY) + !!(subset & AV_CH_LOW_FREQUENCY_2);
}
...
const char *hb_video_encoder_get_name(int encoder) {
  int i;
  for (i = 0; i < hb_video_encoders_count; i++) {
    if (hb_video_encoders[i].item.codec == encoder &&
        hb_video_encoders[i].deprecated == 0) {
      ...
      if (!(audio->config.out.codec & HB_ACODEC_PASS_FLAG)) {
        ...
        if (audio->config.out.mixdown <= 0) {
          audio->config.out.mixdown = hb_mixdown_get_default(
              audio->config.out.codec, audio->config.in.ch_layout);
        } else {
...
static void job_setup(hb_job_t *job, hb_title_t *title) {
  ...
  job->dovi = title->dovi;
  job->passthru_dynamic_hdr_metadata |= title->dovi.dv_profile
                                            ? HB_HDR_DYNAMIC_METADATA_DOVI
                                            : HB_HDR_DYNAMIC_METADATA_NONE;
  job->passthru_dynamic_hdr_metadata |= title->hdr_10_plus
...
int hb_subtitle_extradata_init(hb_subtitle_t *subtitle) {
  ...
  if (subtitle->source == VOBSUB) {
    ...
    sub_idx = hb_strdup_printf(
        ...
        subtitle->width, subtitle->height, 0, 0, "OFF", rgb[0], rgb[1], rgb[2],
        rgb[3], rgb[4], rgb[5], rgb[6], rgb[7], rgb[8], rgb[9], rgb[10],
        rgb[11], rgb[12], rgb[13], rgb[14], rgb[15]);
    if (sub_idx == NULL) {
      return -1;
    }

    hb_set_extradata(&subtitle->extradata, (const uint8_t *)sub_idx,
                     strlen(sub_idx));
    free(sub_idx);
...
int hb_import_subtitle_add(const hb_job_t *job,
  ...
  subtitle->codec_param =
      source == IMPORTSRT ? AV_CODEC_ID_SUBRIP : AV_CODEC_ID_ASS;
...
void hb_metadata_add_coverart(hb_metadata_t *metadata, const uint8_t *data,
                              int size, int type, const char *name) {
  if (metadata) {
...
void hb_compute_chroma_smoothing_coefficient(uint32_t chroma_coeffs[2][4],
  ...
  for (int x = 0; x < 4; x++) {
    chroma_coeffs[0][x] =
        (base_coefficients[x + wX] + base_coefficients[x + wX + !(wX & 0x1)]) >>
        1;
    chroma_coeffs[1][x] =
        (base_coefficients[x + wY] + base_coefficients[x + wY + !(wY & 0x1)]) >>
        1;
  }
}
...
int hb_get_color_prim(int color_primaries, hb_geometry_t geometry,
                      hb_rational_t rate) {
...
static int pix_fmt_is_supported(hb_job_t *job, int pix_fmt) {
  ...
  const AVPixFmtDescriptor *title_desc =
      av_pix_fmt_desc_get(job->title->pix_fmt);
  ...
  for (int i = 0; i < hb_list_count(job->list_filter); i++) {
    ...
    case HB_FILTER_UNSHARP:
    case HB_FILTER_GRAYSCALE:
      if (planes_count == 2 && job->hw_pix_fmt != AV_PIX_FMT_VIDEOTOOLBOX) {
        return 0;
      }
    }
  }
...
static const enum AVPixelFormat pipeline_pix_fmts[] = {
    AV_PIX_FMT_P416,      AV_PIX_FMT_YUV444P16, AV_PIX_FMT_P412,
    AV_PIX_FMT_YUV444P12, AV_PIX_FMT_P410,      AV_PIX_FMT_YUV444P10,
    AV_PIX_FMT_NV24,      AV_PIX_FMT_YUV444P,   AV_PIX_FMT_P212,
    AV_PIX_FMT_YUV422P12, AV_PIX_FMT_P210,      AV_PIX_FMT_YUV422P10,
    AV_PIX_FMT_NV16,      AV_PIX_FMT_YUV422P,   AV_PIX_FMT_P012,
