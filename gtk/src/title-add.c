...
G_MODULE_EXPORT void title_dest_file_cb(GtkWidget *widget,
                                        signal_user_data_t *ud);
G_MODULE_EXPORT void title_dest_dir_cb(GtkWidget *widget, GParamSpec *spec,
                                       signal_user_data_t *ud);
...
static void find_widget_cb(GtkWidget *child, gpointer user_data) {
  find_widget_t *fw = user_data;
  if (fw->result != NULL)
    return;
  fw->result = find_widget(child, fw->name);
}
...
static GtkWidget *find_widget(GtkWidget *widget, const char *name) {
  ...
  if (GTK_IS_CONTAINER(widget)) {
    find_widget_t fw = {name, NULL};
    gtk_container_foreach(GTK_CONTAINER(widget), find_widget_cb, &fw);
    result = fw.result;
  }
#endif
  return result;
...
static gboolean validate_settings(signal_user_data_t *ud, GhbValue *settings,
  ...
  if (dest_exists) {
    ...
    if (!ghb_question_dialog_run(
            hb_window, GHB_ACTION_NORMAL, _("Overwrite"), _("Cancel"),
            _("Overwrite File?"),
            _("Destination: %s\n\n"
...
gboolean ghb_add_title_to_queue(signal_user_data_t *ud, GhbValue *settings,
                                gint batch) {
...
static gboolean title_destination_is_unique(GhbValue *settings_array,
                                            gint index) {
  gint count, ii;
...
static void title_add_set_sensitive(GtkWidget *row, gboolean sensitive) {
  PangoAttrList *pal;
...
static gboolean title_add_are_conflicts(signal_user_data_t *ud) {
  GtkListBox *list;
  GtkWidget *row;
  gint count, ii;
...
static void title_add_set_error_text(signal_user_data_t *ud,
  ...
  if (are_conflicts) {
    msg = _("Duplicate destination files detected. Duplicates will not be "
            "added to the queue.");
  ...
  if (are_conflicts ^ conflict_showing) {
    GtkInfoBar *info =
        GTK_INFO_BAR(ghb_builder_widget("title_add_multiple_infobar"));
    GtkLabel *label =
        GTK_LABEL(ghb_builder_widget("title_add_multiple_label1"));
...
static GtkWidget *title_create_row(signal_user_data_t *ud) {
  ...
  dest_dir = ghb_file_button_new(_("Destination Directory"),
                                 GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
  g_signal_connect(dest_dir, "notify::file", G_CALLBACK(title_dest_dir_cb), ud);
...
G_MODULE_EXPORT void title_add_select_all_cb(GSimpleAction *action,
...
G_MODULE_EXPORT void title_add_clear_all_cb(GSimpleAction *action,
                                            GVariant *param, gpointer data) {
  gint count, ii;
  GhbValue *settings;
  GtkWidget *row;
...
G_MODULE_EXPORT void title_add_invert_cb(GSimpleAction *action, GVariant *param,
                                         gpointer data) {
  gint count, ii;
  GhbValue *settings;
  GtkWidget *row;
...
G_MODULE_EXPORT void title_dest_file_cb(GtkWidget *widget,
  ...
  title_add_set_error_text(ud, title_add_are_conflicts(ud));
}
...
G_MODULE_EXPORT void title_add_action_cb(GSimpleAction *action, GVariant *param,
                                         signal_user_data_t *ud) {
...
static void title_add_multiple_response_cb(GtkDialog *dialog, int response,
                                           signal_user_data_t *ud) {
...
G_MODULE_EXPORT void title_add_multiple_action_cb(GSimpleAction *action,
                                                  GVariant *param,
                                                  signal_user_data_t *ud) {
  GtkListBox *list;
  GtkWidget *row;
...
G_MODULE_EXPORT void title_add_all_action_cb(GSimpleAction *action,
                                             GVariant *param,
                                             signal_user_data_t *ud) {
  gint count, ii;
  GhbValue *preset = NULL;
  ...
  for (ii = 0; ii < count; ii++) {
    if (!title_destination_is_unique(ud->settings_array, ii)) {
      ghb_question_dialog_run(
          GTK_WINDOW(ghb_builder_widget("hb_window")), GHB_ACTION_NORMAL,
          _("OK"), NULL, _("Cannot Add Titles"),
